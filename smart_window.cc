...
namespace agl {
  ...
  SmartWindow::SmartWindow(std::string &name,
                 size_t x, size_t y,
                 size_t w, size_t h) : m_width(w), m_height(h), m_name(name),
                                        m_env(get_env()) {

    static const auto TAG = __func__;

    lg::i(TAG, "creating Window(\"%s\", %zu, %zu, %zu, %zu)",
              name.c_str(), x, y, w, h);
