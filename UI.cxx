...
void Game::drawSettingOnOff(size_t Ycoord, Setting &sg, bool isSelected) const {
  ...
  if(sg.active) {
      str_onoff = (std::string("> ") + sg.txt_on + "      " + sg.txt_off);
  } else {
      str_onoff = (std::string(sg.txt_on) + "      " + "> " + sg.txt_off);
  }
...
void Game::drawSettingItem(size_t Xcoord, size_t Ycoord, const char *name, bool isSelected) const {
    m_env.setColor(isSelected ? agl::YELLOW : agl::WHITE); 
...
void Game::renderMenu() {
  ...
  m_main_win->printOnScreen([&] {
    ...
    for(size_t i = 0; i < N_SETTINGS; ++i) {
      drawSettingOnOff(Ycoord-200 - (i*OFFSET), m_settings.at(i), (m_cur_setting == i));
    }
    // Restart & Quit
    drawSettingItem(Xcoord-100, Ybottom, "Restart", m_cur_setting == N_SETTINGS);
    drawSettingItem(Xcoord+500, Ybottom, "Quit", m_cur_setting == N_SETTINGS+1);
  });
  ...
  const static auto ENTRIES = N_SETTINGS + 2;  // Settings + restart & quit
    ...
    case Key::LEFT: 
      if(m_cur_setting < N_SETTINGS) {
         ...
         lg::i(TAG, "attivato! %s", m_settings.at(m_cur_setting).name);
         lg::i(TAG, "Val %d", m_settings.at(m_cur_setting).active);
      }
      break; 
    ...
    case Key::RIGHT: 
      if(m_cur_setting < N_SETTINGS) {
         ...
         lg::i(TAG, "Disattivato! %s", m_settings.at(m_cur_setting).name);
         lg::i(TAG, "Val %d", m_settings.at(m_cur_setting).active);
      }   break; 
